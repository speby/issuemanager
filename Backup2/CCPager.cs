  ...
  private void Show_Pager()
    {
      ...
      case PagerStyleEnum.Moved:
        ...
        if(StartRange!=1)
          values.Add(new PagingData('<'+(StartRange-1).ToString(), true));
        for(int i=StartRange;(i<(StartRange+NumberOfPages))&&i<=MaxPage;i++)
        {
          values.Add(new PagingData(i.ToString(), CurrentPage!=i));
        }
        if(StartRange+NumberOfPages<=MaxPage)
          values.Add(new PagingData((StartRange+NumberOfPages).ToString()+'>', true));
